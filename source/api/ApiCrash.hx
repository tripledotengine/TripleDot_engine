...
class ApiCrash extends FlxState
{
	...
	override public function create()
	{
		...
		_bg.loadGraphic(Paths.image(ApiFunc._assetpath + 'crash/bg'));
		_bg.setGraphicSize(Std.int(_bg.width * 1.175));
		_bg.screenCenter();
		_bg.alpha = 0.6;
		add(_bg);

		_img1.loadGraphic(Paths.image(ApiFunc._assetpath + 'crash/no'));
		_img1.setPosition(160, FlxG.height - 224);
		_img1.shader = new api.Outline();

		_img2.loadGraphic(Paths.image(ApiFunc._assetpath + 'crash/ok'));
		...
		funcTxt.setFormat(Paths.font('vcr.ttf'), 24, 0xFFFFFFFF, CENTER, OUTLINE, 0xFF000000);
